...
echo "==> Turning event_scheduler ON"
"$MYSQL" "${ARGS[@]}" -e "SET GLOBAL event_scheduler = ON;"
...
echo 'Done.'
echo 'Hint: to see what the recurring events have been writing, run:'
echo '      mysql -u root -p -e "SELECT * FROM event_demo.event_log ORDER BY id DESC;"'